...
def download_vad_model():
    ...
    try:
        ...
        logger.info(
            "Falling back to energy-based VAD - the system will still work but with simpler voice detection"
        )
...
def check_cuda():
    ...
    if torch.cuda.is_available():
        ...
        logger.warning(
            "CUDA is not available. The application will run on CPU, which may be very slow"
        )
