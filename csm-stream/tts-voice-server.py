# will run locally in browser
# talk with an AI like MAYA
...
speaker_counters = {0: 0, 1: 0}  # AI  # User
...
model_id = "openai/whisper-small"  # Changed from large-v3-turbo
# Whisper
...
def load_reference_segments(config_data: CompanionConfig):
    ...
    if os.path.isfile(config_data.reference_audio_path):
        logger.info(
            f"Loading primary reference audio: {config_data.reference_audio_path}"
        )
        wav, sr = torchaudio.load(config_data.reference_audio_path)
        wav = torchaudio.functional.resample(
            wav.squeeze(0), orig_freq=sr, new_freq=24_000
        )
        reference_segments.append(
            Segment(
                text=config_data.reference_text,
        ...
        logger.warning(
            f"Primary reference audio '{config_data.reference_audio_path}' not found."
        )

    # Load second reference (optional)
    if config_data.reference_audio_path2 and os.path.isfile(
        config_data.reference_audio_path2
    ):
        logger.info(
            f"Loading second reference audio: {config_data.reference_audio_path2}"
        )
        ...
        reference_segments.append(
            Segment(
                text=config_data.reference_text2,
                speaker=config_data.voice_speaker_id,
                audio=wav,
            )
        )
    ...
    if config_data.reference_audio_path3 and os.path.isfile(
        config_data.reference_audio_path3
    ):
        logger.info(
            f"Loading third reference audio: {config_data.reference_audio_path3}"
        )
        ...
        reference_segments.append(
            Segment(
                text=config_data.reference_text3,
                speaker=config_data.voice_speaker_id,
                audio=wav,
            )
        )
...
def on_speech_end(audio_data, sample_rate):
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            message_queue.put({"type": "transcription", "text": user_text}), loop
        )

        threading.Thread(
            target=lambda: process_user_input(user_text, session_id), daemon=True
        ).start()
    except Exception as e:
...
def process_user_input(user_text, session_id="default"):
    ...
    if is_speaking:
        logger.info(
            f"AI is currently speaking, adding input to pending queue: '{user_text}'"
        )
    ...
    asyncio.run_coroutine_threadsafe(
        message_queue.put({"type": "status", "message": "Thinking..."}), loop
    )
    ...
    try:
        ...
        threading.Thread(
            target=lambda: rag.add_conversation(user_text, ai_response), daemon=True
        ).start()

        asyncio.run_coroutine_threadsafe(
            message_queue.put({"type": "audio_status", "status": "preparing"}), loop
        )
        ...
        asyncio.run_coroutine_threadsafe(
            message_queue.put({"type": "response", "text": ai_response}), loop
        )
        ...
        if is_speaking:
            logger.warning(
                "Still speaking when trying to start new audio - forcing interrupt"
            )
            interrupt_flag.set()
            is_speaking = False
        ...
        threading.Thread(
            target=audio_generation_thread, args=(ai_response, output_file), daemon=True
        ).start()
...
def start_model_thread():
    ...
    model_thread = threading.Thread(
        target=model_worker, args=(config,), daemon=True, name="model_worker"
    )
    model_thread.start()
    ...
    for client in active_connections[:]:
        try:
            if client.client_state == WebSocketState.CONNECTED:
                ...
                logger.warning(
                    "Detected non-connected client; removing from active_connections"
                )
                active_connections.remove(client)
        except Exception as e:
...
saved_audio_paths = {"default": {0: [], 1: []}}  # AI  # User
MAX_AUDIO_FILES = 8
...
def add_segment(text, speaker_id, audio_tensor):
    ...
    if hasattr(generator, "_text_tokenizer"):
        while dynamic_segments:
            ...
            for segment in temp_full_list:
                tokens = generator._text_tokenizer.encode(
        ...
        logger.warning(
            "Unable to access tokenizer - falling back to word-based estimation for context trimming"
        )
        ...
        while dynamic_segments:
            total_estimated_tokens = sum(
                estimate_tokens(s) for s in protected_segments
    ...
    logger.info(
        f"Context updated. Segments: {len(reference_segments)} total "
...
def audio_generation_thread(text, output_file):
    ...
    if not audio_gen_lock.acquire(blocking=False):
        logger.warning(
            f"Audio generation {this_id} - lock acquisition failed, another generation is in progress"
        )
        asyncio.run_coroutine_threadsafe(
            message_queue.put(
                {
    ...
    try:
        ...
        while True:
            try:
                ...
                if interrupt_flag.is_set():
                    logger.info(
                        f"Audio generation {this_id} - interrupt detected, stopping"
                    )
                ...
                if chunk_counter == 0:
                    ...
                    logger.info(
                        f"Audio generation {this_id} - first chunk latency: {first_chunk_time*1000:.1f}ms"
                    )
                ...
                if interrupt_flag.is_set():
                    logger.info(
                        f"Audio generation {this_id} - interrupt flag set during chunk processing"
                    )
                    break
                ...
                logger.error(
                    f"Audio generation {this_id} - error processing result: {e}"
                )
                break
        ...
        if all_audio_chunks and not interrupt_flag.is_set():
            try:
                ...
                logger.info(
                    f"Audio generation {this_id} - completed in {total_time:.2f}s, RTF: {rtf:.2f}x"
                )
            except Exception as e:
                logger.error(
                    f"Audio generation {this_id} - error saving complete audio: {e}"
                )
        ...
        logger.error(
            f"Audio generation {this_id} - unexpected error: {e}\n{traceback.format_exc()}"
        )
    finally:
        is_speaking = False
        ...
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                message_queue.put(
                    {"type": "audio_status", "status": "complete", "gen_id": this_id}
                ),
                loop,
            )
        except Exception as e:
            logger.error(
                f"Audio generation {this_id} - failed to send completion status: {e}"
            )
...
def handle_interrupt(websocket):
    ...
    time_since_speech_start = (
        current_time - speaking_start_time if speaking_start_time > 0 else 999
    )
    ...
    if time_since_last_interrupt < interrupt_cooldown and time_since_speech_start > 3.0:
        logger.info(
            f"Ignoring interrupt: too soon after previous interrupt ({time_since_last_interrupt:.1f}s < {interrupt_cooldown}s)"
        )
    ...
    if is_speaking or not model_result_queue.empty():
        ...
        # Notify clients
        asyncio.run_coroutine_threadsafe(
            message_queue.put({"type": "audio_status", "status": "interrupted"}), loop
        )

        asyncio.run_coroutine_threadsafe(
            websocket.send_json(
                {"type": "audio_status", "status": "interrupt_acknowledged"}
            ),
            loop,
        )
            ...
            if data["type"] == "config":
                ...
                try:
                    ...
                    if saved:
                        ...
                        await websocket.send_json(
                            {"type": "error", "message": "Failed to save configuration"}
                        )

                except Exception as e:
                    logger.error(f"Error processing config: {str(e)}")
                    await websocket.send_json(
                        {"type": "error", "message": f"Configuration error: {str(e)}"}
                    )
                ...
                if success:
                    ...
                    with user_input_lock:
                        if pending_user_inputs:
                            ...
                            threading.Thread(
                                target=lambda: process_user_input(
                ...
                await websocket.send_json(
                    {"type": "mute_status", "muted": data["muted"]}
                )
...
async def get_conversations(request: Request):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, user_message, ai_message FROM conversations ORDER BY id DESC"
    )
    data = [
        {"id": row[0], "user_message": row[1], "ai_message": row[2]}
...
def update_conversation(conv_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE conversations SET user_message=?, ai_message=? WHERE id=?",
...
if __name__ == "__main__":
    ...
    threading.Thread(
        target=lambda: asyncio.run(loop.run_forever()), daemon=True
    ).start()
