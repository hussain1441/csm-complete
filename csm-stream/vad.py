...
class VoiceActivityDetector:
    def __init__(
        ...
        print(
            f"VAD initialized with threshold {threshold}, frame size {self.frame_size}, silence duration {silence_duration}"
        )
...
class AudioStreamProcessor:
    ...
    def process_audio(self, audio_chunk: np.ndarray):
        ...
        if self.vad.is_speaking and not self.is_collecting:
            ...
            print(
                f"Speech started, beginning collection with {len(self.pre_speech_buffer)} pre-speech chunks"
            )
        ...
        if self.is_collecting and not self.vad.is_speaking:
            ...
            print(
                f"Silent chunk count: {self.silent_chunk_count}, max: {self.max_silent_chunks}"
            )
            # Force end after too many silent chunks
            if self.silent_chunk_count >= self.max_silent_chunks:
                is_turn_end = True
                print(
                    f"Forcing speech end after {self.silent_chunk_count} silent chunks"
                )
        else:
            self.silent_chunk_count = 0
                ...
                if "on_speech_end" in self.callbacks:
                    try:
                        ...
                        self.callbacks["on_speech_end"](
                            complete_audio, self.sample_rate
                        )
